...
def imgdir_list(root_dir):
    all_files = glob.glob(os.path.join(root_dir, '*'))
    # all_files = os.listdir(root_dir)
    image_extensions = ['.jpg', '.jpeg', '.png', '.bmp', '.gif', '.tif', '.tiff']
    img_dirs = [file for file in all_files if os.path.splitext(file)[1].lower() in image_extensions]
    img_dirs.sort()
    return img_dirs


def imgcv2tensor(imgs, bgr2rgb=True):
...
def calculate_ssim(imgA, imgB, gray_scale=False):
    ...
    if gray_scale:
        score = SSIM(cv2.cvtColor(imgA, cv2.COLOR_RGB2GRAY), cv2.cvtColor(imgB, cv2.COLOR_RGB2GRAY))
    else:
        score = SSIM(imgA, imgB, full=True, multichannel=True)
    return score
