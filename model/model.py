...
class DDPM(BaseModel):
    def __init__(self, opt):
        ...
        self.set_loss()
        self.ca = opt['model']['ca']
        self.set_new_noise_schedule(opt['model']['beta_schedule']['train'], schedule_phase='train')
        self.set_new_noise_schedule(opt['model']['beta_schedule']['val'], schedule_phase='val')
    ...
    def feed_data(self, data):
        ...
        if self.opt['dist']:
            ...
            dic['GT'] = data['GT']
            if self.ca:
                dic['low_edge'] =data['le']
                dic['low_semantic'] =data['ls']
                dic['high_edge'] =data['he']
                dic['high_semantic'] =data['hs']
    ...
    def optimize_parameters(self):
        ...
        if not self.opt['uncertainty_train']:
            if self.opt['dist']:
                l_pix, l_gsad = self.netG(self.data, None)
            else:
                l_pix, l_gsad = self.netG(self.data, None)
            ...
            if isinstance(self.netG, nn.DataParallel):
                ...
                if self.opt['dist']:
                    ...
                    if self.opt['model']['ca']:
                        self.SR = self.netG.super_resolution(self.data['LQ'], continous,q=[self.data['low_edge'],self.data['low_semantic']])
                    else:
                        self.SR = self.netG.super_resolution(self.data['LQ'], continous,q=None)
    ...
    def set_new_noise_schedule(self, schedule_opt, schedule_phase='train'):
        ...
        if self.opt['dist']:
            ...
            if self.schedule_phase is None or self.schedule_phase != schedule_phase:
                ...
                if isinstance(self.netG, nn.DataParallel):
                    self.netG.module.set_new_noise_schedule(
                        schedule_opt, self.device,schedule_phase)
                else:
                    self.netG.set_new_noise_schedule(schedule_opt, device,schedule_phase)
            ...
            if isinstance(self.netG, nn.DataParallel):
                ...
                self.netG.set_new_noise_schedule(schedule_opt, self.device,schedule_phase)
    ...
    def save_network(self, epoch, iter_step,savename = None):
        ...
        if savename is not None:
            gen_path = os.path.join(
                self.opt['path']['checkpoint'], '{}_gen.pth'.format(savename))
            opt_path = os.path.join(
                self.opt['path']['checkpoint'], '{}_opt.pth'.format(savename))
